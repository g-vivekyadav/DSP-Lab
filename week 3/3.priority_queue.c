...
void insert()
{
    ...
    if (rear >= size - 1)
    {
    	...
    	//gate score should be in between 0 to 1000
    ...
    int i,j;
    //pushing the data and rearrange the data after priority of gate score
    for (i = 0; i <= rear; i++)
    {
        if (tempdata > array[i])
        {
...
void Delete()
{
    ...
    //display the top of the priority elements and popping
    printf("%s      %d\n",name[0],array[0]);
    for (i=0; i < rear; i++)
    {
        array[i] = array[i + 1];
